...
class Skrill_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function is_enabled() {
		...
		if ( ! empty( $payment_settings ) &&
			is_array( $payment_settings ) &&
			! empty( $payment_settings['enabled'] ) &&
			'yes' === $payment_settings['enabled']
		) {
			return true;
		}
	...
	public function is_show_separately() {
		...
		if ( ! empty( $payment_settings ) &&
			is_array( $payment_settings ) &&
			! empty( $payment_settings['show_separately'] ) &&
			'yes' === $payment_settings['show_separately']
		) {
			return true;
		}
	...
	public function render_additional_information() {
		if ( ! self::$added_meta_boxes ) {
			...
			if ( !$order_id ) {
				$order_id = Skrill_Payment::get_request_value( 'id' );
			}
	...
	protected function change_order_status() {
		...
		if ( $transaction['payment_status'] === $this->skrill_pending_status
		...
		} elseif ( $transaction['payment_status'] === $this->skrill_refund_pending_status
			...
			$this->redirect_order_detail( $error_message );
		} elseif ( ( $transaction['payment_status'] === $this->skrill_processed_status
			|| $transaction['payment_status'] === $this->skrill_refund_failed_status )
			&& $order_post_status === $this->wc_refunded_status
		) {
			$is_refunded_payment =
			...
			$this->increase_order_stock();
		} elseif ( $transaction['payment_status'] === $this->skrill_pending_status
